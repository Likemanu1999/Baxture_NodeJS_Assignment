...
import { NonService, NonList } from './non-service';
...
@Component({
	selector: 'app-lc-in-operation',
	templateUrl: './lc-in-operation.component.html',
	styleUrls: ['./lc-in-operation.component.css'],
...
export class LcInOperationComponent implements OnInit {
	...
	updatePiQtyTolerance(pi_id, pi_qty) {
		...
		if (confirm('Are you Sure ? Do you want to Adjust Tolereance for  this PI')) {
	...
	onSubmitReviseCopy() {
		...
		if (!this.reviseNonForm.invalid) {
			...
			if (document.length > 0) {
				...
				this.nonService.updateReviseNon(formData).subscribe((response) => {
					this.toasterService.pop(response.message, response.message, response.data);
					if (response.code === '100') {
	...
	saveDataReviseCopy(form) {
		this.nonService.updateReviseNon(form).subscribe((response) => {
			this.toasterService.pop(response.message, response.message, response.data);
			if (response.code === '100') {
				this.ReviseNonModal.hide();
				this.reviseNonForm.reset();
	...
	onSubmitNon() {
		...
		if (status === true) {
			...
			if (document.length > 0) {
				...
				if (pi_qty.length > 0) {
					this.nonService.addNon(formData).subscribe((response) => {
						this.toasterService.pop(response.message, response.message, response.data);
						if (response.code === '100') {
	...
	updateNon() {
		if (this.nonid) {
			...
			this.nonService.updateNon(formData).subscribe((response) => {
				this.toasterService.pop(response.message, response.message, response.data);
				if (response.code === '100') {
					this.nonReset();
					this.nonModal.hide();
	...
	onDeleteNon(id: number) {
		if (id) {
			this.nonService.deleteNon(id).subscribe((response) => {
				if (response.code === '100') {
					this.toasterService.pop(response.message, response.message, response.data);
					this.getDetails(this.lc_id);
				}
