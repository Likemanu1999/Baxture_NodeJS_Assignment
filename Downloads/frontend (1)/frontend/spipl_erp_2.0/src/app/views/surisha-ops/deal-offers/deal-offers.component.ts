...
@Component({
	...
	styleUrls: ['./deal-offers.component.scss'],
...
export class DealOffersComponent implements OnInit {
	...
	page_title: any = "Primary Deal Offer";
	role_id: any = null;
	company_id: any = null;
	links: string[] = [];
	cols: any = [];
	...
	getCols() {
		this.cols = [
			{ field: "action", header: "Action", sort: false, filter: false, dropdown: [], footer: false, total: 0, type: null, style: '9%' },
			{ field: "offer_id", header: "Offer ID", sort: true, filter: true, dropdown: [], footer: false, total: 0, type: null, style: '7%' },
			...
			{ field: "information", header: "Information", sort: false, filter: false, dropdown: [], footer: false, total: 0, type: null, style: '7%' },
			{ field: "arrival", header: "Arrival", sort: false, filter: false, dropdown: [], footer: false, total: 0, type: null, style: '7%' },
			{ field: "offer_end", header: "Offer Valid Till", sort: false, filter: false, dropdown: [], footer: false, total: 0, type: "Date", style: '15%' },
			{ field: "offer_rate", header: "Offer Rate", sort: false, filter: false, dropdown: [], footer: false, total: 0, type: null, style: '7%' },
			...
			{ field: "term", header: "Delivery", sort: true, filter: true, dropdown: [], footer: false, total: 0, type: null, style: '7%' },
			{ field: "name", header: "Country", sort: true, filter: true, dropdown: [], footer: false, total: 0, type: null, style: '7%' },
			{ field: "sector", header: "Sector", sort: true, filter: false, dropdown: [], footer: false, total: 0, type: null, style: '7%' },
		];

		this.filter = ['supplier','offer_id', 'grade_name', 'portName', 'term', 'name'];
		this.getData();
	}
	...
	onAction(item: any, type: string) {
		switch (type) {
			case 'Add Deal':
				this.router.navigate(["surisha-ops/add-deal"]);
				break;

			case 'Edit':
				this.router.navigate(["surisha-ops/edit-deal", JSON.stringify(item)]);
				break;
	...
	deleteOffer(data) {
		this.crudServices.deleteData(dealOffer.delete, data).subscribe(result => {
			this.toasterService.pop('success', 'Success', result['data'].message);
			this.getData();
		});
	}
}
