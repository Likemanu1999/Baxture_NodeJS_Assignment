...
export class ChatsComponent implements OnInit, AfterViewChecked {
  ...
  getChatHeadCount() {
    ...
    this.whatsapp.getChatHeads('api/whatsapp/getChatHeadCount', data).subscribe(response => {
      if (response) {
      ...
      if (data.company != 'Surisha') {
        if (data.chat_type == 'newChat') { // For new chat (new chat key)
          if (this.heads.length == 0) {
            ...
            // data.map(item => { // when single head not
  ...
  getHeads(data) {
    ...
    this.whatsapp.getChatHeads('api/whatsapp/getChatHeads', data).subscribe(response => {
  ...
  sendMsg() {
    if (this.message) {
      this.target = false;
      this.whatsapp.sendMsg('api/whatsapp/sendChatMessage', { message: this.message, to: this.contact_number, userId: this.userId }).subscribe(response => {
        this.message = null
