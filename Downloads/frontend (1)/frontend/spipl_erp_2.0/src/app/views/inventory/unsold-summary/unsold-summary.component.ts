...
@Component({
	...
	styleUrls: ['./unsold-summary.component.css', 'unsold-summary.scss'],
...
export class UnsoldSummaryComponent implements OnInit {
	...
	one_godown_all_grade_forward_sales_pending = 0;
	...
	constructor(private CrudServices: CrudServices, private toasterService: ToasterService, private loginService: LoginService) {
		...
		this.CrudServices.getRequest<any>(forexReports.getAllGradeWithCompanyId).subscribe((response) => {
			...
			// console.log(this.main_grade_master, "this.main_grade");
			// console.log(this.main_grade, "this.main_grade ngoninit");

			// this.main_grade.forEach(element => {
			// 	this.main_grade_ids.push(element.id)
			// })
			// console.log(this.main_grade_ids, "this.main_grade_ids ngoninit");
			let main_grade_array = [];
			this.main_grade.forEach((element) => {
				main_grade_array.push(element.id);
			})
			// console.log(main_grade_array, "this.main_grade_array ngoninit");
			...
			// 		return a.indexOf(e) === i;
			// 	});
			// 	// this.main_grade_ids = [4, 5, 8, 9, 10, 2, 3, 1, 11, 13, 6, 7, 14];
	...
	getUnsoldList() {
		...
		if (this.company_Ids.length == 0) {
			this.onCompanyChange([{ "id": 0, "name": "All" }])
			this.company_Ids = [0];
		}
		...
		console.log('company ids', this.company_Ids.length == 0);
		console.log('main_grade_ids', this.main_grade_ids.length == 0);


		this.CrudServices.postRequest<any>(forexReports.getUnsoldSummary, {
			main_grade_id_array: this.main_grade_ids,
			company_id_array: this.company_Ids
		}).subscribe((response) => {
			console.log(response, "unsold");
			this.data = response;
			this.testData = this.data['port_wise_details'];
			this.pi_Regular_Import_Sum = Number(this.data['lc_to_be_given']) + Number(this.data['total_forward_pending'])
			this.testData.unshift({
				'port': 'PI To Be Called',
				'pnforRegular': this.data['lc_to_be_given'],
				'pnforInport': this.data['total_forward_pending'],
				port_id: 0,
				main_godown_id: 0,
				lc_not_issue: 0,
				non_pending: 0,
				reg_arrival: 0,
				bond_arrival: 0,
				stock_transfer_intransite: 0,
				local_purchase: 0,
				godwon_stock: 0,
				sales_pending: 0,
				forward_sales_pending: 0,
				totalQuantity: this.pi_Regular_Import_Sum,
				totalUnsold: this.pi_Regular_Import_Sum
			})
	...
	getDetailUnsold(main_godown_id, port_name) {
		console.log('Main Godown Id', main_godown_id, 'Port Name', port_name);
		this.isLoading = true;
		...
		this.CrudServices.postRequest<any>(forexReports.getDetailUnsoldSummary, {
			...
			// main_grade_id_array: this.main_godown_id_Set,
			company_id_array: this.company_Ids,
			main_grade_id_array: this.main_grade_ids
		}).subscribe((response) => {
			...
			this.one_godown_all_grade_forward_sales_pending = 0;
			let mergedObject = [];
			...
			for (let item of response) {
				...
				let frwd_sale_pen_agn_grade = 0;
				let mainGrade;
				for (let elem of item[1]) {

					let inhand = (elem.nonpending ? elem.nonpending : 0) + (elem.reg_intransit ? elem.reg_intransit : 0) +
						(elem.bond_intransit ? elem.bond_intransit : 0) + (elem.local_intransit ? elem.local_intransit : 0) +
						(elem.stock_transfer_intransite ? elem.stock_transfer_intransite : 0) + (elem.inventory ? elem.inventory : 0) +
						(elem.lc_not_issue ? elem.lc_not_issue : 0);

					total_inhand = total_inhand + inhand;
					sale_pen_agn_grade = sale_pen_agn_grade + (elem.sales_pending ? elem.sales_pending : 0);
					frwd_sale_pen_agn_grade = frwd_sale_pen_agn_grade + (elem.total_forward_sales_pending ? elem.total_forward_sales_pending : 0);
					...
					this.one_godown_all_grade_forward_sales_pending = this.one_godown_all_grade_forward_sales_pending + (elem.total_forward_sales_pending ? elem.total_forward_sales_pending : 0)

				}
				// console.log(sale_pen_agn_grade, "sale_pen_agn_grade")
				item[2] = total_inhand;
				item[3] = total_inhand - (sale_pen_agn_grade + frwd_sale_pen_agn_grade);
				item[4] = mainGrade;

				var resultObject = item[1].reduce(function (result, currentObject) {
				...
				Object.assign(resultObject, {
					'totalInhand': item[2],
					'inhandSaleDiff': item[3],
					'mainGrade': mainGrade
				});

				mergedObject.push(resultObject);
			}

			this.Detaildata = mergedObject;
	...
	onCompanyChange(event: Array<any>) {
		...
		// console.log('main_grade after Company change Event', this.main_grade);
