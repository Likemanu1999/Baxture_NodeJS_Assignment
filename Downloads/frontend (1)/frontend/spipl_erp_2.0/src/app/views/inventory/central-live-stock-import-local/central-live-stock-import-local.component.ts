...
@Component({
	selector: 'app-central-live-stock-import-local',
	templateUrl: './central-live-stock-import-local.component.html',
	styleUrls: ['./central-live-stock-import-local.component.scss'],
	encapsulation: ViewEncapsulation.None,
	providers: [CrudServices, ToasterService, PermissionService, LoginService, ExportService]

})
export class CentralLiveStockImportLocalComponent implements OnInit {
				...
				for (let item, i = 0; item = this.data[i++];) {
					...
					if (item.grade_details.length) {
						...
						for (let sub_item, j = 0; sub_item = item.grade_details[j++];) {
							...
							this.tot_unsold_import = this.tot_unsold_import + (sub_item.bond_intransite + Number(sub_item.stock_transfer_intransite_import) + sub_item.inventory_import) + Number(sub_item.local_purchase_import_intransite);

							// sub_item.reg_intransite + 
							sub_item.total_unsold = (sub_item.bond_intransite + Number(sub_item.stock_transfer_intransite_import) + sub_item.inventory_import + Number(sub_item.local_purchase_import_intransite));
							...
							// sub_item.reg_intransite +
							this.tot_available_import = this.tot_available_import + ((sub_item.bond_intransite + Number(sub_item.stock_transfer_intransite_import) + sub_item.inventory_import + Number(sub_item.local_purchase_import_intransite)) - (sub_item.sales_pending_import));

							//sub_item.reg_intransite + 
							sub_item.available_import = ((sub_item.bond_intransite + Number(sub_item.stock_transfer_intransite_import) + sub_item.inventory_import + Number(sub_item.local_purchase_import_intransite)) - (sub_item.sales_pending_import));

							this.tot_hold_import = this.tot_hold_import + Number(sub_item.hold_qty_import);

							//sub_item.reg_intransite +
							this.inventory_after_hold_import = this.inventory_after_hold_import + (((sub_item.bond_intransite + Number(sub_item.stock_transfer_intransite_import) + sub_item.inventory_import + Number(sub_item.local_purchase_import_intransite)) - (sub_item.sales_pending_import)) - Number(sub_item.hold_qty_import));

							//sub_item.reg_intransite + 
							sub_item.stock_after_hold_import = (((sub_item.bond_intransite + Number(sub_item.stock_transfer_intransite_import) + sub_item.inventory_import + Number(sub_item.local_purchase_import_intransite)) - (sub_item.sales_pending_import)) - Number(sub_item.hold_qty_import));
	...
	onFilter(event, dt) {
		...
		if (this.filteredValuess) {
			...
			for (const val of this.filteredValuess) {
				...
				//val.reg_intransite +
				this.tot_unsold_import = this.tot_unsold_import + (val.bond_intransite + Number(val.stock_transfer_intransite_import) + val.inventory_import + Number(val.local_purchase_import_intransite));
				this.tot_sales_pending_import = this.tot_sales_pending_import + val.sales_pending_import;
				//val.reg_intransite +
				this.tot_available_import = this.tot_available_import + ((val.bond_intransite + Number(val.stock_transfer_intransite_import) + val.inventory_import + Number(val.local_purchase_import_intransite)) - (val.sales_pending_import));
				this.tot_hold_import = this.tot_hold_import + Number(val.hold_qty_import);
				//val.reg_intransite + 
				this.inventory_after_hold_import = this.inventory_after_hold_import + (((val.bond_intransite + Number(val.stock_transfer_intransite_import) + val.inventory_import + Number(val.local_purchase_import_intransite)) - (val.sales_pending_import)) - val.hold_qty_import);
