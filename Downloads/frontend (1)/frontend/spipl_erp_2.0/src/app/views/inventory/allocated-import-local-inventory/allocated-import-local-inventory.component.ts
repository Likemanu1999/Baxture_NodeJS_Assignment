...
@Component({
	selector: 'app-allocated-import-local-inventory',
	templateUrl: './allocated-import-local-inventory.component.html',
	styleUrls: ['./allocated-import-local-inventory.component.scss'],
	encapsulation: ViewEncapsulation.None,
	providers: [CrudServices, PermissionService, LoginService, ExportService, ToasterService]

})
export class AllocatedImportLocalInventoryComponent implements OnInit {
	...
	loadData() {
		...
		this.data = [];
		this.AllData = [];
		this.CrudServices.getRequest<any>(LiveInventory.marketing_person_wise_stock_import_local).subscribe((response) => {
			...
			if (response.length > 0) {
				...
				for (let item, i = 0; item = response[i++];) {
					...
					for (let grd_item, j = 0; grd_item = item.grade_details[j++];) {
						...
						console.log(grd_item.isAllocateLocal, local_import_flag_name, "before")
						...
						let import_allocated = ((Number(grd_item.tot_bond_intransite) + Number(grd_item.tot_stock_transfer_intransite_import) + Number(grd_item.tot_inventory_import) - Number(grd_item.tot_sales_pending_import)) - Number(grd_item.tot_hold_qty_import));

						let local_allocated = (Number(grd_item.tot_local_intransite) + Number(grd_item.tot_stock_transfer_intransite_local) + Number(grd_item.tot_inventory_local) - Number(grd_item.tot_sales_pending_local)) - Number(grd_item.tot_hold_qty_local);
						...
						if (grd_item.isAllocateImport == 1) {
							...
							this.AllData.push({
								...
								is_allocate: grd_item.isAllocateImport,
								main_godown_id: main_godown_id
							});
						}
						...
						if (grd_item.isAllocateLocal == 1) {
							...
							this.AllData.push({
								...
								is_allocate: grd_item.isAllocateLocal,
								main_godown_id: main_godown_id
							});
						}
					}
