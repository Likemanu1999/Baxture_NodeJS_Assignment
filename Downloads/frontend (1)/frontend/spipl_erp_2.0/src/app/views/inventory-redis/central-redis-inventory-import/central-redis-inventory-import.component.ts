...
@Component({
	selector: 'app-central-redis-inventory-import',
	templateUrl: './central-redis-inventory-import.component.html',
	styleUrls: ['./central-redis-inventory-import.component.scss'],
	providers: [CrudServices, ToasterService, LoginService, ExportService]
})
export class CentralRedisInventoryImportComponent implements OnInit {
	...
	default_hold_allocate_set: boolean = false;
	links: string[] = [];
	...
	constructor(private CrudServices: CrudServices, private toasterService: ToasterService, private loginService: LoginService, private exportService: ExportService) {
		...
		this.default_hold_allocate_set = (this.links.indexOf('default_hold_allocate_set') > -1);
		...
		this.CrudServices.getRequest<any>(forexReports.getAllGradeWithCompanyId).subscribe((response) => {
			...
			this.CrudServices.postRequest<any>(forexReports.getUnsoldSummaryImportLocalNewFiltered, {
				...
				console.log(this.resRegIntransite, this.resMaterialInBondPending, "nehaaa")
		...
		this.cols = [
			...
			{ field: "regular_intransite_spipl", header: "Material Under Clearance", permission: true, style: '100px' },
			{ field: "bond_intransite_spipl", header: "Material In Bond", permission: true, style: '100px' },
			{ field: "stock_transfer_import_instransite", header: "Stock Transfer Intransit", permission: true, style: '100px' },
			{ field: "local_purchase_import_intransite", header: "Purchase In Transit", permission: true, style: '100px' },
			{ field: "inventory_import", header: "Godown Inventory", permission: true, style: '100px' },
			{ field: "totalQuantity", header: "Total Quantity", permission: true, style: '100px' },
			{ field: "sales_pending_import", header: "Order Pending", permission: true, style: '100px' },
			...
			{ field: "hold_qty_import", header: "Hold Import", permission: true, style: '100px' },
	...
	getImportHoldQty() {
		this.CrudServices.postRequest<any>(inventoryRedis.getAllHoldImportLocal, {
			...
			this.holdQtyArr = response.data.filter(val => val.hold_qty_import != 0);
		...
		this.CrudServices.postRequest<any>(inventoryRedis.getUnsoldSummaryRedis, {
			...
			for (let item of finalArr) {
				...
				if (holdImportResult.length > 0) {
					item.hold_qty_import = holdImportResult[0].hold_qty_import
				} else {
					item.hold_qty_import = 0
				}

				let allocateImportResult = this.allocateArr.filter(val => val.main_godown_id == item.main_godown_id && val.grade_id == item.grade_id)
				if (allocateImportResult.length > 0) {
					item.is_allocate_import = allocateImportResult[0].is_allocate_import;
				} else {
					item.is_allocate_import = 0;
				}


				this.tot_hold_qty_import = this.tot_hold_qty_import + item.hold_qty_import;


				if (!this.pi_flag_hold_release) {

					filteredArrayPortWise = this.resRegIntransite.filter((reg_item) => reg_item.godown_id == item.main_godown_id && reg_item.grade_id == item.grade_id
					);
					...
					filteredArrayPortWise = this.resMaterialInBondPending.filter((bond_item) => bond_item.godown_id == item.main_godown_id && bond_item.grade_id == item.grade_id);
				...
				item.totalQuantity = (
					item.bond_intransite_spipl +
					item.stock_transfer_import_instransite +
					item.local_purchase_import_intransite +
					item.inventory_import
				);
	...
	onFilter(event, dt) {
		...
		if (this.filteredValuess) {
			...
			for (const elem of this.filteredValuess) {
				...
				this.tot_sales_pending_local = this.tot_sales_pending_local + elem.sales_pending_local;
				this.tot_sales_pending_import_surisha = this.tot_sales_pending_import_surisha + elem.sales_pending_import_surisha;
				this.tot_hold_qty_import = this.tot_hold_qty_import + elem.hold_qty_import;
	...
	holdQtyUpdate(main_godown_id, grade_id, hold_qty) {
		...
		this.CrudServices.postRequest<any>(inventoryRedis.getholdImportLocal, {
			...
			this.hold_qty_input = response.data[0] ? response.data[0].hold_qty_import : 0;
	...
	submitHold() {
		...
		//In Redis HoldUpdate Update Live stock hold
	...
	OnCheckGodownAllocationImport(checked, item) {
		...
		//In Redis allocate Update Live stock allocate
	...
	set_default_allocate_hold() {
		this.CrudServices.getRequest<any>(inventoryRedis.setDefaultAllocateHold).subscribe((response) => {
	...
	exportData() {
		...
		for (let j = 0; j < this._selectedColumns.length; j++) {
			...
			if (this._selectedColumns[j]['field'] === 'regular_intransite_spipl') {
				foot[this._selectedColumns[j]['header']] = this.tot_regular_intransite_spipl;
			} else if (this._selectedColumns[j]['field'] === 'bond_intransite_spipl') {
				foot[this._selectedColumns[j]['header']] = this.tot_bond_intransite_spipl;
			}
			else if (this._selectedColumns[j]['field'] === 'stock_transfer_import_instransite') {
				foot[this._selectedColumns[j]['header']] = this.tot_stock_transfer_import_instransite;
			}
				...
				foot[this._selectedColumns[j]['header']] = this.tot_inventory_import;
			}


			else if (this._selectedColumns[j]['field'] === 'totalQuantity') {
				foot[this._selectedColumns[j]['header']] = (this.tot_bond_intransite_spipl + this.tot_stock_transfer_import_instransite + this.tot_local_purchase_import_intransite + this.tot_inventory_import);
			}
				...
				foot[this._selectedColumns[j]['header']] = (
					(
						this.tot_bond_intransite_spipl +
						this.tot_stock_transfer_import_instransite + this.tot_local_purchase_import_intransite +
						this.tot_inventory_import
					) - (this.tot_sales_pending_import)
				)
			}
				...
				foot[this._selectedColumns[j]['header']] = this.tot_hold_qty_import;
			}
