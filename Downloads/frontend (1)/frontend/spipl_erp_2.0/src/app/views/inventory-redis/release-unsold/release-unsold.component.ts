...
import { crons, forexReports, inventoryRedis } from '../../../shared/apis-path/apis-path';
...
@Component({
	selector: 'app-release-unsold',
	templateUrl: './release-unsold.component.html',
	styleUrls: ['./release-unsold.component.scss'],
	providers: [CrudServices, ToasterService, LoginService]
})
export class ReleaseUnsoldComponent implements OnInit {
	...
	constructor(private CrudServices: CrudServices, private toasterService: ToasterService, private loginService: LoginService) {
		...
		this.whatsapp = (this.links.indexOf('pvc_unsold_release_whastapp') > -1);
			...
			this.CrudServices.postRequest<any>(forexReports.getUnsoldSummaryImportLocalNewFiltered, {
				...
				finalArr = finalArr.map(item => {
					if (!this.pi_flag_hold_release) {
						...
						console.log(item.non_pending_spipl, 'nonpending', filteredArrayPortWise.reduce((accumulator, object) => {
						...
						console.log(item.regular_intransite_spipl, 'regularIntransit', filteredArrayPortWise.reduce((accumulator, object) => {
							return accumulator + object.pending_qty;
						}, 0));
	...
	getDetailUn(main_godown_id) {
		...
		let data = data1.filter(val => val.main_godown_id == main_godown_id && val.company_id == 1);
		...
		this.CrudServices.postRequest<any>(forexReports.getUnsoldSummaryImportLocalNewFiltered, {
			...
			this.tot_regular_intransite_spipl_det = 0;
			this.tot_bond_intransite_spipl_det = 0;
			console.log(responseHold, 'array');
			...
			finalArr.map(item => {
				...
				if (!this.pi_flag_hold_release) {
					...
					//&& lcNotIssue_item.grade_id == item.grade_id
					filteredArrayPortWise = this.reslcNotIssue.filter((lcNotIssue_item) => lcNotIssue_item.godown_id == item.main_godown_id && lcNotIssue_item.grade_id == item.grade_id && lcNotIssue_item.main_grade_id == item.main_grade_id
					);

					console.log(filteredArrayPortWise, 1);
					...
					// && nonPending_item.grade_id == item.grade_id
					filteredArrayPortWise = this.resNonPending.filter((nonPending_item) => nonPending_item.godown_id == item.main_godown_id && nonPending_item.grade_id == item.grade_id && nonPending_item.main_grade_id == item.main_grade_id
					);
					...
					//&& reg_item.grade_id == item.grade_id
					filteredArrayPortWise = this.resRegIntransite.filter((reg_item) => reg_item.godown_id == item.main_godown_id && reg_item.grade_id == item.grade_id && reg_item.main_grade_id == item.main_grade_id
					);

					console.log(item.regular_intransite_spipl, filteredArrayPortWise);
					...
					//&& bond_item.grade_id == item.grade_id
					filteredArrayPortWise = this.resMaterialInBondPending.filter((bond_item) => bond_item.godown_id == item.main_godown_id && bond_item.grade_id == item.grade_id && bond_item.main_grade_id == item.main_grade_id);
					console.log(filteredArrayPortWise, 'bomd');
					item.bond_intransite_spipl = item.bond_intransite_spipl - filteredArrayPortWise.reduce((accumulator, object) => {
						return accumulator + object.pending_qty;
					}, 0);

					this.tot_regular_intransite_spipl_det = this.tot_regular_intransite_spipl_det + item.regular_intransite_spipl;
					this.tot_bond_intransite_spipl_det = this.tot_bond_intransite_spipl_det + item.bond_intransite_spipl;

					this.tot_lc_not_issue_spipl_det = this.tot_lc_not_issue_spipl_det + item.lc_not_issue_spipl;
					this.tot_non_pending_spipl_det = this.tot_non_pending_spipl_det + item.non_pending_spipl;
	...
	onSubmit() {
		if (this.selectedMobileNo.length && this.main_grade_ids.length) {
			console.log(this.selectedMobileNo);
			this.CrudServices.postRequest<any>(crons.usoldPvcReleaseWhatsapp, { mobile_list: this.selectedMobileNo, main_grade: this.main_grade_ids }).subscribe(response => {
				this.toasterService.pop('success', 'success', 'Whatsapp Sent')
			...
			this.toasterService.pop('error', 'error', 'select main Grade')
		}
	}
}
