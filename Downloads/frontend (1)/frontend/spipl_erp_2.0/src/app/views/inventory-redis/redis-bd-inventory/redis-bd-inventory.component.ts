...
@Component({
	selector: 'app-redis-bd-inventory',
	templateUrl: './redis-bd-inventory.component.html',
	styleUrls: ['./redis-bd-inventory.component.scss'],
	providers: [CrudServices, ToasterService, PermissionService, LoginService, ExportService]
})
export class RedisBdInventoryComponent implements OnInit {
	...
	loadData() {
		...
		this.isLoading = true;
		this.data = [];
		this.CrudServices.postRequest<any>(LiveInventory.getPvcPolyInventoryRedis, { main_grade_ids: this.main_grade_ids }).subscribe((response) => {
				...
				for (let item, i = 0; item = this.data[i++];) {
					...
					if (item.grade_details.length) {
						...
						for (let sub_item, j = 0; sub_item = item.grade_details[j++];) {
							...
							sub_item.stock_transfer_intransite_num = (Number(sub_item.stock_transfer_import_instransite) + Number(sub_item.stock_transfer_local_instransite));

							sub_item.sales_pending = sub_item.sales_pending_import + sub_item.sales_pending_local
							...
							this.tot_import_pur_reg = this.tot_import_pur_reg + sub_item.regular_intransite_spipl;

							this.tot_order_in_stock = this.tot_order_in_stock + (sub_item.local_purchase_import_intransite + sub_item.local_purchase_local_intransite);

							this.tot_import_pur_bond = this.tot_import_pur_bond + sub_item.bond_intransite_spipl;
							this.tot_stock_tranfer = this.tot_stock_tranfer + (Number(sub_item.stock_transfer_import_instransite) + Number(sub_item.stock_transfer_local_instransite));
							...
							this.tot_physical_stock = this.tot_physical_stock + (sub_item.inventory_import + sub_item.inventory_local);

							// sub_item.reg_intransite +
							this.tot_unsold = this.tot_unsold + (Number(sub_item.bond_intransite_spipl) + (Number(sub_item.stock_transfer_import_instransite) + Number(sub_item.stock_transfer_local_instransite)) + Number(sub_item.order_in_stock) + (Number(sub_item.inventory_import) + Number(sub_item.inventory_local)));

							// sub_item.reg_intransite + 
							sub_item.total_unsold = ((sub_item.arrival_grade ? sub_item.arrival_grade : 0) + sub_item.bond_intransite_spipl + (Number(sub_item.stock_transfer_import_instransite) + Number(sub_item.stock_transfer_local_instransite)) + sub_item.order_in_stock + (sub_item.inventory_import + sub_item.inventory_local));


							this.tot_sales_pending = this.tot_sales_pending + (sub_item.sales_pending_import + sub_item.sales_pending_local);

							// sub_item.reg_intransite +
							this.tot_available = this.tot_available + (((sub_item.arrival_grade ? sub_item.arrival_grade : 0) + sub_item.bond_intransite_spipl + (Number(sub_item.stock_transfer_import_instransite) + Number(sub_item.stock_transfer_local_instransite)) + sub_item.order_in_stock + (sub_item.inventory_import + sub_item.inventory_local)) - (sub_item.sales_pending_import + sub_item.sales_pending_local));

							//sub_item.reg_intransite + 
							sub_item.available = (((sub_item.arrival_grade ? sub_item.arrival_grade : 0) + sub_item.bond_intransite_spipl + (Number(sub_item.stock_transfer_import_instransite) + Number(sub_item.stock_transfer_local_instransite)) + sub_item.order_in_stock + (sub_item.inventory_import + sub_item.inventory_local)) - (sub_item.sales_pending_import + sub_item.sales_pending_local));



							this.tot_arrival_till_date = this.tot_arrival_till_date + (sub_item.arrival_grade ? sub_item.arrival_grade : 0);

							sub_item.instransite = ((sub_item.arrival_grade ? sub_item.arrival_grade : 0) + sub_item.bond_intransite_spipl +
	...
	onFilter(event, dt) {
		...
		if (this.filteredValuess) {
			...
			for (const val of this.filteredValuess) {
				this.tot_import_pur_reg = this.tot_import_pur_reg + val.regular_intransite_spipl;
				this.tot_order_in_stock = this.tot_order_in_stock + (val.local_purchase_import_intransite + val.local_purchase_local_intransite);
				this.tot_import_pur_bond = this.tot_import_pur_bond + val.bond_intransite_spipl;
				this.tot_stock_tranfer = this.tot_stock_tranfer + (Number(val.stock_transfer_local_instransite) + Number(val.local_purchase_import_intransite));
				this.tot_local_pur = this.tot_local_pur + val.order_in_stock;
				this.tot_physical_stock = this.tot_physical_stock + (val.inventory_import + val.inventory_local);

				//val.reg_intransite +
				this.tot_unsold = this.tot_unsold + ((val.arrival_grade ? val.arrival_grade : 0) + val.bond_intransite_spipl + (Number(val.stock_transfer_import_instransite) + Number(val.stock_transfer_local_instransite)) + val.order_in_stock + (val.inventory_import + val.inventory_local));


				this.tot_sales_pending = this.tot_sales_pending + (val.sales_pending_import + val.sales_pending_local);
				//val.reg_intransite +
				this.tot_available = this.tot_available + (((val.arrival_grade ? val.arrival_grade : 0) + val.bond_intransite_spipl + (Number(val.stock_transfer_import_instransite) + Number(val.stock_transfer_local_instransite)) + val.order_in_stock + (val.inventory_import + val.inventory_local)) - (val.sales_pending_import + val.sales_pending_local));


				this.tot_arrival_till_date = this.tot_arrival_till_date + (val.arrival_grade ? val.arrival_grade : 0);
				this.tot_intransite = this.tot_intransite + ((val.arrival_grade ? val.arrival_grade : 0) + val.bond_intransite_spipl + (Number(val.stock_transfer_import_instransite) + Number(val.stock_transfer_local_instransite)));
