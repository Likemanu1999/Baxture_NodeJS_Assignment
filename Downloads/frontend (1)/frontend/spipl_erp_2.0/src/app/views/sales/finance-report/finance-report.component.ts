...
import { SalesReportsNew, LocalPurchase, FileUpload } from '../../../shared/apis-path/apis-path';
...
@Component({
	...
	providers: [CrudServices, ToasterService, PermissionService, ExportService, CurrencyPipe],
})
...
export class FinanceReportComponent implements OnInit {
	...
	tab_1_title = "Group Outstanding Report";
	tab_2_title = "Credit Limit Report";
	cols: any = [];
	data: any = [];
	filter: any = [];
	cols2: any = [];
	...
	getOutstandingReport() {
		this.data = [];
		this.isLoading = true;
		this.crudServices.getOne<any>(SalesReportsNew.getGroupOutstandingReport, {
	...
	getCols(value) {
		if (this.showModal) {
			...
			if (this.selected_tab == 0) {
				...
				this.filter2 = ['sub_org_name', 'zone'];
				...
				this.cols = [
					...
					{ field: "sub_org_name", header: "Customer", sort: true, filter: true, dropdown: [], footer: false, total: 0, type: null },
					{ field: "zone", header: "Zone", sort: true, filter: true, dropdown: [], footer: false, total: 0, type: null },
					{ field: "credit_limit", header: "Credit Limit", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "utilized_credit", header: "Utilized Credit", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "overdue_amount", header: "Overdue Amount", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "available_credit", header: "Available Credit", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "adhoc_limit", header: "Adhoc Credit", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "adhoc_limit_utilized", header: "Adhoc Utilized", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "adhoc_available", header: "Adhoc Available", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "overdue_limit", header: "Overdue Credit", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "overdue_limit_utilized", header: "Overdue Utilized", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "overdue_available", header: "Overdue Available", sort: true, filter: false, dropdown: [], footer: true, total: 0, type: "Amount" },
					{ field: "case", header: "Case", sort: true, filter: false, dropdown: [], footer: false, total: 0, type: null },
				];
				this.getCreditLimitReport();
			}
	...
	onFilter(e, dt) {
		this.getTotal(dt.filteredValue);
	...
	onAction(item, type) {
		if (type == 'Tab') {
			...
			if (numbers.length) {
				let main_obj = {
					locale: "en",
					template_name: (this.company_id == 3) ? "payment_overdue_reminder_surisha" : "payment_overdue_reminder",
					numbers: numbers,
	...
	exportData(type) {
		...
		let final_data = this.data;
		if (this.filterValuess == null) {
			final_data = this.data;
		} else {
			final_data = this.filterValuess;
		}
		if (this.showModal) {
	...
	sendWtsapp(data, path) {
		let percent = (data[0]['product_type'] == 1) || (data[0]['product_type'] == 3) ? 24 : 36;
		let main_obj_for_cust = {
			locale: "en",
			template_name: (data[0]['product_type'] == 3) ? "overall_outstanding_payments_surisha" : "overall_outstanding_payments",
			attachment: [
				{
