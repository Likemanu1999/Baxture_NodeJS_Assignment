...
@Component({
  selector: 'app-logistics-charges-list',
  templateUrl: './logistics-charges-list.component.html',
  styleUrls: ['./logistics-charges-list.component.scss'],
...
export class LogisticsChargesListComponent implements OnInit {
  ...
  complete_storage_charges: any;
  complete_storage_qty: any;
  ...
  columInitialize() {
    this.cols = [
      ...
      { field: 'storageQty', header: 'Storage Quantity', style: '100' ,  total : this.complete_storage_qty},
      { field: 'storage', header: 'Storage Charges', style: '100' ,  total : this.complete_storage_charges},
        ...
        for(let element of response) {
          ...
          if(element.bill_of_lading != null) {
            ...
            if(element.bill_of_lading.logistics_storage_charges  != null) {
              ...
              storage = element.bill_of_lading.logistics_storage_charges.reduce((sum, item) => sum + (Number(item.storage_charges) + Number(item.storage_sgst) + Number(item.storage_cgst)), 0);

              storageQty = element.bill_of_lading.logistics_storage_charges.reduce((sum, item) => sum + (Number(item.storage_qty)), 0);
            }
  ...
  calculateTotal(be_list) {
    ...
    this.complete_storage_qty = be_list.reduce((sum, item) => sum + Number(item.storageQty), 0);
    this.complete_storage_charges = be_list.reduce((sum, item) => sum + Number(item.storage), 0);
