...
import { EmailTemplateMaster, FileUpload, Lot_coa, NonNegotiable, percentage_master, PortMaster, shipmentType, SubOrg } from '../../../shared/apis-path/apis-path';
import { Router } from '@angular/router';
import * as moment from 'moment';


@Component({
  selector: 'app-purchase-register-import-report-new',
  templateUrl: './purchase-register-import-report-new.component.html',
  styleUrls: ['./purchase-register-import-report-new.component.scss'],
...
export class PurchaseRegisterImportReportNewComponent implements OnInit {
  ...
  data: { invoice_no: any; shippine_line_id: any; import_type: any; bl_no: any; be_no: any; lot_no: any; };
  totalQty: any;
  selectedPort = [];
  port = 1;
  port_id: any;
  ...
  constructor(private router: Router, private toasterService: ToasterService,
    ...
    console.log(this.upload_endorse_copy);

    this.division=staticValues.companies;
    ...
    this.cols = [
      ...
      { field: 'name', header: 'Division', style: '200' }, 
  ...
  getNonList() {
    ...
    this.crudServices.getOne<any>(NonNegotiable.getMaterialArrivalDet, filter).subscribe(response => {
      ...
      for (let element of response) {
        ...
        if (element.grade_master.main_grade.product_master.company_id != null) {
          let companyId = element.grade_master.main_grade.product_master.company_id
          if (companyId == 1) { element.name = "PVC" } else if (companyId == 2) { element.name = "PE & PP" } else if (companyId == 3) { element.name = "SURISHA" }
        }
        ...
        element.charges_per_mt = Number(element.total_cha_cfs_shipping_terminal / element.tot_non_qty).toFixed(2);
        element.charges_per_cont = Number(total_cha_cfs_shipping_terminal /
          noc).toFixed(2)
        ...
        element.charges = `Total Charges/MT:
				...
				: ${element.storage_charges_mt} `;
        // this.totalQty += element.tot_non_qty;
        // this.totNOC += element.noc;
        // this.totCharges += element.total_cha_cfs_shipping_terminal;
      ...
      // 
      // this.port = this.non_list[0].port_id;
      // this.selectedPort.push(this.ports[0]);
      ...
      console.log(this.gloobalList, "this.gloobalList")
      this.CalculateTotal(this.non_list);
  ...
  onchange(event, name) {
    ...
    if (event.value.length > 0) {
      for (let i = 0; i < event.value.length; i++) {
        ...
        console.log("array push >>",event.value[i][name]);
    ...
    for (let i = 0; i < arr.length; i++) {
      ...
      for (let j = 0; j < this._selectedColumns.length; j++) {
        if (this._selectedColumns[j]['field'] == 'port_name') {
          ...
          export_data[this._selectedColumns[j]['header']] = arr[i]['invoice_no'] + date;
        }
  ...
  exportExcel() {
    this.export_purchase_list = [];
    this.exportData();
    console.log(this.export_purchase_list, 'DATTTTA');
